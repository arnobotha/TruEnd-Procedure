...
if (!exists('datCredit_real')) unpack.ffdf(paste0(genPath,"creditdata_final2-TruEnd"), tempPath)
...
suppressWarnings({
  datCredit_real[, `:=`(DefaultStatus1 = NULL, DelinqState_g0 = NULL)]   
})

# - Subset information for fusion with main dataset later
datTemp_Final <- datTemp2[, list(LoanID, Counter, DelinqState_g0, DefaultStatus1)]

# - Intermediate cleanup | Memory optimisation
rm(datTemp2, lstCurables, matg0); gc()
...
datCredit_real <- merge(datCredit_real, 
                        datTemp_Final[, list(LoanID, Counter, DelinqState_g0, DefaultStatus1)],
...
# --- 1.4. Reorder new features created thus far
...
pack.ffdf(paste0(genPath,"creditdata_final2b-TruEnd"), datCredit_real)
...
# - Confirm prepared credit data is loaded into memory
if (!exists('datCredit_real')) unpack.ffdf(paste0(genPath,"creditdata_final2b-TruEnd"), tempPath)
...
# - Save to disk (zip) for quick disk-based retrieval later
pack.ffdf(paste0(genPath,"creditdata_final2c-TruEnd"), datCredit_real)
...
# - Confirm prepared credit data is loaded into memory
if (!exists('datCredit_real')) unpack.ffdf(paste0(genPath,"creditdata_final2c-TruEnd"), tempPath)
...
# - Save to disk (zip) for quick disk-based retrieval later
pack.ffdf(paste0(genPath,"creditdata_final3-TruEnd"), datCredit_real)
