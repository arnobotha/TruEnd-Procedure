...
MAE_combined_surv <- subset(Combined_imp[Time<=120,], select = c(Time, Surv_KM, Dataset))
MAE_combined_surv2 <- MAE_combined_surv %>% pivot_wider(names_from = c(Dataset), values_from = c(Surv_KM))
MAE_combined_surv2 <- na.omit(MAE_combined_surv2)
MAE_combined_surv2 <- MAE_combined_surv2 %>% mutate(MAE = abs(MAE_combined_surv2$NoTruEnd - MAE_combined_surv2$TruEnd))
(diag.impact_Def <- mean(abs(MAE_combined_surv2$NoTruEnd - MAE_combined_surv2$TruEnd)) * 100)
### RESULTS ~ 0.692375
ggplot(MAE_combined_surv2, aes(x=Time, y=MAE)) + theme_bw() +
...
MAE_combined_cum_haz <- subset(Combined_imp[Time<=120,], select = c(Time, CumulHazard, Dataset ))
MAE_combined_cum_haz2 <- MAE_combined_cum_haz %>% pivot_wider(names_from = c(Dataset), values_from = c(CumulHazard))
MAE_combined_cum_haz2 <- na.omit(MAE_combined_cum_haz2)
MAE_combined_cum_haz2 <- MAE_combined_cum_haz2 %>% mutate(MAE = abs(MAE_combined_cum_haz2$TruEnd - MAE_combined_cum_haz2$NoTruEnd))
#MAE_combined_cum_haz2 <- MAE_combined_cum_haz2[Time<=240,]
(diag.impact_Def <- mean(abs(MAE_combined_cum_haz2$NoTruEnd - MAE_combined_cum_haz2$TruEnd)) * 100)
### RESULTS ~ 2.155924
ggplot(MAE_combined_cum_haz2, aes(x=Time, y=MAE)) + theme_bw() +
  geom_line(linetype=1, color="Red") +
  theme(panel.grid.minor.x = element_blank())

# - Hazard functions
MAE_combined_haz <- subset(Combined_imp[Time<=120,], select = c(Time, hazard, Dataset ))
MAE_combined_haz2 <- MAE_combined_haz %>% pivot_wider(names_from = c(Dataset), values_from = c(hazard))
MAE_combined_haz2 <- na.omit(MAE_combined_haz2)
MAE_combined_haz2 <- MAE_combined_haz2 %>% mutate(MAE = abs(MAE_combined_haz2$NoTruEnd - MAE_combined_haz2$TruEnd))
#MAE_combined_haz2 <- MAE_combined_haz2[Time<=240,]
(diag.impact_Def <- mean(abs(MAE_combined_haz2$NoTruEnd - MAE_combined_haz2$TruEnd)) * 100)
### RESULTS ~ 0.06871158
ggplot(MAE_combined_haz2, aes(x=Time, y=MAE)) + theme_bw() +
  geom_line(linetype=1, color="Blue") +
  theme(panel.grid.minor.x = element_blank())
...
(gsurv1c_TruEnd_a <- ggsurvplot(kmDef_woff_real_spell1c_TruEnd, fun="pct", conf.int=T, legend="none", 
                         ...
                         cumevents=T, cumevents.title="Cumulative number of write-offs", 
                         cumcensor=T, cumcensor.title="Cumulative number of censored observations (incl. competing risks)",
                         risk.table.title = "Number in (% of) sample at risk of being written off", font.family=chosenFont, fontsize=2.5))
...
ggsave(print(gsurv1c_TruEnd_a,newpage=F), file=paste0(graphPath_real,"/SurvFig1c_TruEnd_a-WOFF_Surv-KaplanMeier-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
ggsave(print(gsurv1c_TruEnd_b,newpage=F), file=paste0(graphPath_real,"/SurvFig1c_TruEnd_b-WOFF_CumulEvent-KaplanMeier-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
ggsave(print(gsurv1c_TruEnd_c,newpage=F), file=paste0(graphPath_real,"/SurvFig1c_TruEnd_c-WOFF_CumulHaz-NelsonAalen-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
dpi <- 180 # reset
ggsave(gsurv1c_TruEnd_d, file=paste0(graphPath_real,"/SurvFig1c_TruEnd_d-WOFF_Hazard-KaplanMeier-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)

# - cleanup
#rm(datSurv_TruEnd); gc()
...
# - save plots
dpi <- 120 # need to decrease size for risk tables' text
ggsave(print(gsurv1c_NoTruEnd_a,newpage=F), file=paste0(graphPath_real,"/SurvFig1c_NoTruEnd_a-WOFF_Surv-KaplanMeier-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
ggsave(print(gsurv1c_NoTruEnd_b,newpage=F), file=paste0(graphPath_real,"/SurvFig1c_NoTruEnd_b-WOFF_CumulEvent-KaplanMeier-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
ggsave(print(gsurv1c_NoTruEnd_c,newpage=F), file=paste0(graphPath_real,"/SurvFig1c_NoTruEnd_c-WOFF_CumulHaz-NelsonAalen-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
dpi <- 180 # reset
ggsave(gsurv1c_NoTruEnd_d, file=paste0(graphPath_real,"/SurvFig1c_NoTruEnd_d-WOFF_Hazard-KaplanMeier-SpellLevel-MultiSpell-LatentComp-InclLeftTrunc_Correct.png"),width=1200/dpi, height=1000/dpi,dpi=dpi)
